...
from module import get_activation_function
...
class NeighGraphEncoderDecoder(nn.Module):
    def __init__(self,pointset,
        ...
        self.num_neg_resample = num_neg_resample
...
class GlobalPositionNeighGraphEncoderDecoer(nn.Module):
    """
    ...
    def __init__(self,pointset,
                    ...
                    num_context_sample=10,
                    num_neg_resample=10):
        super(GlobalPositionNeighGraphEncoderDecoer,self).__init__()
        self.pointset = pointset
        self.enc = enc
        self.init_dec = init_dec
        self.dec = dec
        ...
        self.activation = get_activation_function(activation,"GlobalPositionNeighGraphEncoderDecoer")
...
class GlobalPositionEncoderDecoder(nn.Module):
    ...
    def get_neg_pt_embed(self, ng_list, do_full_eval = True):
        ...
        if do_full_eval == True:
            num_neg_sample = len(ng_list[0].neg_samples)
            # print("num_neg_sample:",num_neg_sample)
            # pt_list: shape (batch_size*num_neg_sample)
            pt_list = []
            for ng in ng_list:
                pt_list += list(ng.neg_samples)
